        ...
        (err, history, cachedHistory) => {
            ...
            market.getItemOrdersHistogram(
                ...
                (err, histogram, cachedListings) => {
                    ...
                    logConsole(`出售价格：${sellPrice / 100.0} (手续费后：${market.getPriceIncludingFees(sellPrice) / 100.0})`);
...
function initializeInventoryUI() {
    ...
    let previousSelection = -1; // 保存上一次选中的索引
    ...
    getInventorySelectedMarketableItems((items) => {
        ...
        if (items.length == 0) {
            ...
            if (canSellSelectedItemsManually(items)) {
                $('.sell_manual').show();
                $('.sell_manual > span').text(`手动出售 ${selectedItems} 个物品`);
            } else {
                $('.sell_manual').hide();
            }
            $('.sell_selected > span').text(`出售 ${selectedItems} 个物品`);
        }
    });
}
...
function updateTurnIntoGemsButton() {
    getInventorySelectedGemsItems((items) => {
        ...
        if (items.length == 0) {
            ...
            $('.turn_into_gems').show();
            $('.turn_into_gems > span').text(`将 ${selectedItems} 个物品转化为宝石`);
        }
    });
}
...
function updateOpenBoosterPacksButton() {
    getInventorySelectedBoosterPackItems((items) => {
        ...
        if (items.length == 0) {
            ...
            $('.unpack_selected_booster_packs > span').text(`打开 ${selectedItems} 个补充包`);
        }
    });
}
...
async function updateInventorySelection(selectedItem) {
    ...
    if (item_info.html().indexOf('checkout/sendgift/') > -1) { // 礼物没有市场信息
        return;
    }
...
market.getItemOrdersHistogram(
    ...
    (err, histogram) => {
        if (err) {
            logConsole(`获取 ${selectedItem.name || selectedItem.description.name} 订单数据失败`);
            return;
        }
        ...
        if (selectedItem.queued != null) {
            ...
            $('.quick_sell').on('click', function () {
                ...
                sellQueue.push({ item: selectedItem, sellPrice: price });
            });
...
function updateInventoryUI(isOwnInventory) {
    ...
    $('#see_settings').remove();
    $('#global_action_menu').
        prepend('<span id="see_settings"><a href="javascript:void(0)">⬖ Steam 市场增强工具</a></span>');
    ...
    if (showMiscOptions) {
        buttonsHtml += `
                    ...
                    <div class="see_inventory_buttons">
                        <a class="btn_darkblue_white_innerfade btn_medium_wide turn_into_gems" style="display:none"><span>将选中物品转化为宝石</span></a>
                        <a class="btn_darkblue_white_innerfade btn_medium_wide unpack_all_booster_packs"><span>打开所有补充包</span></a>
                        <a class="btn_darkblue_white_innerfade btn_medium_wide unpack_selected_booster_packs" style="display:none"><span>打开选中补充包</span></a>
                        <a class="btn_darkblue_white_innerfade btn_medium_wide gem_all_duplicates"><span>将所有重复物品转化为宝石</span></a>
                    </div>
                `;
    } else if (TF2) {
    ...
    $('#inventory_applogo').hide(); // 隐藏Steam/游戏logo，避免重复显示
...
function getInventoryItems() {
    ...
    // 部分库存（如 BattleBlock Theater）没有子库存，只有一个主库存
...
if (currentPage == PAGE_MARKET || currentPage == PAGE_MARKET_LISTING) {
    ...
    function marketListingsQueueWorker(listing, ignoreErrors, callback) {
      ...
      const market_hash_name = getMarketHashName(asset);
            ...
            if (listingUI == null) {
                logConsole(`未在列表中找到挂单 ${listing.listingid}，跳过`);
            ...
            if (price <= getSettingWithDefault(SETTING_PRICE_MIN_CHECK_PRICE) * 100 || listingUI.hasClass('removing')) {
                ...
                $('.market_listing_my_price', listingUI).last().prop('title', '价格未检查');
...
market.getPriceHistory(
    ...
    (errorPriceHistory, history, cachedHistory) => {
        if (errorPriceHistory) {
            logConsole(`获取 ${game_name} 价格历史失败`);
        ...
        market.getItemOrdersHistogram(
            ...
            (errorHistogram, histogram, cachedListings) => {
                if (errorHistogram) {
                    logConsole(`获取 ${game_name} 订单数据失败`);
                ...
                // 在市场挂单上显示最高求购价
                // 'histogram.highest_buy_order' 不可靠，因为Steam会缓存该值，但对旧游戏/挂单有一定参考意义
                ...
                $(
                    ...
                    listingUI
                ).append(` ➤ <span title="这可能是当前最高求购价。">${highestBuyOrderPrice
                    }</span>`);
                ...
                // 这里计算两个价格：一个不带加价偏移，一个带加价偏移
                // 不带偏移的价格用于：当你已是最低价时（例如使用负偏移），避免不停重复上架
                // 带偏移的价格用于：最终上架，确保应用用户设置的偏移值
                ...
                const sellPriceWithoutOffsetWithFees = market.getPriceIncludingFees(sellPriceWithoutOffset);
                            ...
                            logConsole(`计算价格：${sellPriceWithoutOffsetWithFees / 100.0} (${sellPriceWithoutOffset / 100.0})`);

                            listingUI.addClass(`price_${sellPriceWithOffset}`);

                            $('.market_listing_my_price', listingUI).last().prop(
                                'title',
                                `最优价格为 ${formatPrice(sellPriceWithoutOffsetWithFees)}。`
                            );
        ...
        function marketOverpricedQueueWorker(item, ignoreErrors, callback) {
            ...
            if (listingUI == null) {
                logConsole(`未在列表中找到挂单 ${item.listing}，跳过`);
            ...
            market.removeListing(
                ...
                (errorRemove) => {
                    if (!errorRemove) {
                        ...
                        }, getRandomInt(1500, 2500)); // 短暂等待，确保物品已返回库存
                    } else {
                        $('.actual_content', listingUI).css('background', COLOR_ERROR);
                        return callback(false);
                    }
    ...
    $('.market_home_listing_table').each(function (e) {
        ...
        // 不处理热门/新上架/近期售出物品（页面底部）
        if ($('.my_market_header', $(this)).length == 0) {
            return;
        }

        // 求购单和挂单确认页不像出售单那样分组，添加此处理使分页也能生效
        if (!$(this).attr('id')) {
    ...
    marketLists.flatMap(list => list.items).forEach(item => {
        ...
        if (isSellOrder) {
            ...
            if (assetInfo.appid === undefined) {
                logConsole(`跳过挂单 ${listingid}（未找到游戏ID）`);
                return;
            }
        ...
        if (isBuyOrder) {
            ...
            if (assetInfo.amount === undefined) {
                logConsole(`跳过挂单 ${listingid}（未找到数量）`);
                return;
            }
        ...
        logConsole(`跳过物品 ${item.elm.id}（非求购/出售挂单）`);
    });
...
function processMarketListings() {
    ...
    if (currentPage == PAGE_MARKET) {
        ...
        $('#tabContentsMyActiveMarketListingsRows').html(''); // 清空默认挂单
        $('#tabContentsMyActiveMarketListingsRows').hide(); // 加载完成前隐藏所有挂单
...
function updateMarketSelectAllButton() {
    $('.market_listing_buttons').each(function () {
        ...
        if ($('.market_select_item', selectionGroup).length == 0) { // 如果没有可选项，保持显示“全选”
            invert = false;
        }
        // 按钮文字切换：全选 / 取消全选
...
function sortMarketListings(elem, isPrice, isDateOrQuantity, isName) {
    ...
    if (isName) {
        ...
        function getListingFromLists(listingid) {
            // 有时挂单ID会出现在多个列表中，使用最后一个，通常是我们需要的
...
$('#see_settings').remove();
$('#global_action_menu').prepend('<span id="see_settings"><a href="javascript:void(0)">⬖ Steam 市场增强工具</a></span>');
$('#see_settings').on('click', '*', () => openSettings());
...
if (currentPage == PAGE_TRADEOFFER) {
    ...
    function sumTradeOfferAssets(assets, user) {
        ...
        let totalText = `<strong>独特物品数量：${sortable.length}，总价值 ${formatPrice(totalPrice)}<br/><br/></strong>`;
    ...
    const appendSelectPageButton = () => {
        $('#inventory_displaycontrols').append(`<div class="trade_offer_buttons">
          <a class="item_market_action_button item_market_action_button_green select_all">
              <span class="item_market_action_button_contents" style="text-transform:none">选择本页全部物品</span>
          </a>
      </div>`);
...
const price_options = $(`<div id="see_settings_modal">
    <div>
        ...
        <select id="${SETTING_PRICE_ALGORITHM}">
            <option value="1"${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 1 ? 'selected="selected"' : ''}>历史均价与最低售价取最大值</option>
            <option value="2" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 2 ? 'selected="selected"' : ''}>直接使用最低售价</option>
            <option value="3" ${getSettingWithDefault(SETTING_PRICE_ALGORITHM) == 3 ? 'selected="selected"' : ''}>当前最高求购价 或 最低售价</option>
        </select>
    </div>
...
unsafeWindow.ShowConfirmDialog('Steam 市场增强工具', price_options).done(() => {
